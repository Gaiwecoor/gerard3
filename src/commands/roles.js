  ...
  try {
    ...
    if (user && user.bhid) {
      let {
        ...
        clanRole,
        clanId,
      } = await Module.db.server.getSettings(guild.id);
      ...
      if (roles.length > 0) {
        let removeRoles = []
          .concat(clanRole, rankedRoles, regionRoles)
          .filter(
            (r, i, a) =>
              !roles.includes(r) &&
              member.roles.has(r) &&
        ...
        msg.reply("I couldn't find any roles to give you.").then(u.clean);
    } else if (msg)
      msg.reply("you need to `claim` your account first.").then(u.clean);
  } catch (e) {
    u.alertError(e, msg);
  }
  if (msg) u.clean(msg);
...
const Module = new Augur.Module()
  ...
  .addCommand({
    ...
    category: "Profile",
    permissions: (msg) =>
      canManage(msg) && !Module.db.server.getSetting(msg.guild, "enforceRoles"),
    process: (msg) => {
      let { regionRoles, rankedRoles, clanRole } = Module.db.server.getSettings(
        msg.guild.id
      );
      let roles = []
