...
Module = new Augur.Module()
  .addCommand({
    ...
    description:
      "Find upcoming official Brawlhalla tournaments on [Smash.gg](https://smash.gg/brawlhalla).",
    process: async (msg) => {
      try {
        let embed = u
          .embed()
          ...
          .setDescription(
            "Official Brawlhalla Tournaments on [Smash.gg](https://smash.gg/brawlhalla)"
          )
          .setThumbnail(
        ...
        let circuit = await smash.tournament("brawlhalla", [
          "visibleEntityContainerTag",
        ]);
        ...
        tourneys
          .map((t) => t.entities.tournament)
          .forEach((t) => {
            let d = new Date(t.startAt * 1000);
            embed.addField(
              d.toDateString(),
              `[${t.name}](https://smash.gg/${t.slug})`
            );
          });
  ...
  .addCommand({
    ...
    category: "Tournaments",
    description:
      "Find upcoming Brawlhalla tournaments on [Smash.gg](https://smash.gg/tournaments?per_page=30&filter=%7B%22upcoming%22%3Atrue%2C%22videogameIds%22%3A%2215%22%2C%22past%22%3Afalse%7D&page=1)",
    ...
    process: async (msg) => {
      try {
        let embed = u
          ...
          .setDescription(
            "See all official Brawlhalla tournaments on [Smash.gg](https://smash.gg/brawlhalla)"
          )
          .setThumbnail(
            Module.config.imgPath +
              "/weapons/spawn.png?API_KEY=" +
        ...
        let date = Math.floor(
          (Date.now() + 4 * 7 * 24 * 60 * 60 * 1000) / 1000
        );
