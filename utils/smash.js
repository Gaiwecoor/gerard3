...
const SmashApi = {
	callApi: function(call, id, expands=[], method="get") {
		...
		return new Promise(function(fulfill, reject) {
			url = "https://api.smash.gg/" + call + "/" + id + ((expands.length > 0) ? ("?expand[]=" + expands.join("&expand[]=")) : "");
	...
	top_bracket: function(phaseId, expands=["sets", "entrants"]) {
		var self = this;
		return new Promise(function(fulfill, reject) {
			self.callApi("phase", phaseId, ["groups"]).then(phase => {
	...
	search: function(options) {
		var self = this;
		return new Promise(function(fulfill, reject) {
			self.callApi("public/tournaments", `schedule?filter=${JSON.stringify(options)}`).then(fulfill, reject);
		});
	}
};
