...
function rankedEmbed(rank, index = 0, count = null) {
  ...
  if (!rank)
    return embed.addField(
      "Error",
      "Sorry, I ran into an error fetching ranked results. Please try again in a moment."
    );
  ...
  rank.winrate =
    rank.games > 0 ? ((100 * rank.wins) / rank.games).toFixed(1) : 0;
  if (rank["2v2"]) {
  ...
  for (var x in rankInfo) {
    if (rank.tier.toLowerCase().startsWith(x)) {
      embed
        .setColor(rankInfo[x].color)
        .setThumbnail(
          `${config.imgPath}/ranked/${rankInfo[x].image}.png?API_KEY=${config.api.resource}`
        );
      break;
    }
  }
  ...
  let name =
    `**[${u.decode(rank.name)}](https://brawldb.com/player/stats/${
      rank.brawlhalla_id
    })**` +
    (rank.clan
      ? `\n< [${u.decode(rank.clan.clan_name)}](https://brawldb.com/clan/info/${
          rank.clan.clan_id
        }) >`
      : "");

  embed
    .setTitle(`${rank.verified ? "☑️ Verified " : ""}Ranked Data`) // for ${u.decode(rank.name)}`)
  ...
  if (rank.legends)
    embed.addField(
      "Legends",
      `**Highest Rating:** ${
        bh.legendSummaries.get(bestLegend.legend_id).bio_name
      }\n**Most Played:** ${
        bh.legendSummaries.get(rank.legends[0].legend_id).bio_name
      }`,
      true
    );
  ...
  if (glory.games >= 10) {
    ...
    glory.winsAward = GetGloryFromWins(glory.wins);
  ...
  if (count > 1)
    embed.setDescription(
      `Result ${index + 1} of ${count}. React with ◀ or ▶ within ${
        time / 60000
      } minutes to view other results.`
    );
  ...
  */
  ...
  embed
    .addField(
      "__Ranked Play__",
      glory.games >= 10
        ? `**Estimated Glory:** ${glory.ratingAward + glory.winsAward}`
        : "\u200B"
    )
    .addField(
      "1v1 Rating",
      `**${rank.tier}** (${rank.rating} / ${rank.peak_rating} Peak)\n${rank.wins} Wins / ${rank.loss} Losses (${rank.games} Games)\n${rank.winrate}% Winrate` +
        (rank.global_rank ? `\n**Global Rank** ${rank.global_rank}` : "") +
        (rank.region_rank
  ...
  if (rank.team) {
    ...
    rank.team.winrate =
      rank.team.games > 0
        ? ((100 * rank.team.wins) / rank.team.games).toFixed(1)
        : 0;

    try {
      embed.addField(
        "2v2 Team Rating",
        `**${u.decode(rank.team.teamname)}**\n**${rank.team.tier}** (${
          rank.team.rating
        } / ${rank.team.peak_rating} Peak)\n${rank.team.wins} Wins / ${
          rank.team.loss
        } Losses (${rank.team.games} Games)\n${rank.team.winrate}% Winrate`,
        true
      );
    } catch (e) {
      console.error(e);
...
function statEmbed(stats) {
  ...
  let embed = u
    .embed()
    .setTitle(
      `${stats.verified ? "☑️ Verified " : ""}Brawlhalla Stats for ${u.decode(
        stats.name
      )}`
    )
    .addField(
      "Name",
      `**[${u.decode(stats.name)}](https://brawldb.com/player/stats/${
        stats.brawlhalla_id
      })**` +
        (stats.clan
          ? `\n< [${u.decode(
              stats.clan.clan_name
            )}](https://brawldb.com/clan/info/${stats.clan.clan_id}) >`
          : ""),
      true
    )
    .addField(
      "Overall",
      [
        `${stats.wins} Wins / ${stats.games - stats.wins} Losses (${
          stats.games
        } Games)`,
    ...
    .addField(
      ...
      [
        "**Most Used:** " +
          (response.legend.time ? response.legend.time.name : "None"),
        "**Highest Winrate:** " +
          (response.legend.wr
            ? `${response.legend.wr.name} (${response.legend.wr.value}%)`
            : "None"),
        "**Highest Avg DPS:** " +
          (response.legend.dps
            ? `${response.legend.dps.name} (${response.legend.dps.value})`
            : "None"),
        "**Shortest Avg TTK:** " +
          (response.legend.ttk
            ? `${response.legend.ttk.name} (${response.legend.ttk.value}s)`
            : "None"),
      ].join("\n"),
      true
    )
...
async function updateRankedEmbed(msg) {
  try {
    ...
    if (
      ...
      msg.channel.send(
        msg.author +
          ", my system requires `Embed Links` and `Add Reactions` permissions for me to function properly and it looks like I don't have those. Try talking to the server owner to make sure I have the permissions I need."
      );
...
const Module = new Augur.Module()
  .addCommand({
    ...
    usage: "[@user]",
    info:
      "Retrieves rankings for a mentioned user (if they have `claim`ed their profile) or the Steam name you specify.",
        ...
        if (target) {
          ...
          if (
            ...
            if (rank.rating) {
              ...
              let leaderboard = await bh.getRankings({
              ...
              embed = statEmbed(stats).setDescription(
            ...
            channel.send(embed).catch((e) => {
              if (
                ...
                channel.send(
                  msg.author +
                    ", my system requires `Embed Links` permissions for me to function properly and it looks like I don't have those. Try talking to the server owner to make sure I have the permissions I need."
                );
              else u.alertError(e, msg);
            });
            ...
            msg
              .reply("that user hasn't `claim`ed their profile.")
              .then(u.clean);
          }
        } else {
          // Search by name
          ...
          if (results.length > 0) {
            ...
            msg
              .reply(`I couldn't find any results for ${suffix}.`)
              .then(u.clean);
        }
      } catch (e) {
        u.alertError(e, msg);
    ...
    process: async (msg, suffix) => {
      try {
        ...
        if (bhid) {
          ...
          msg
            .reply("you need to tell me which Brawlhalla ID to search!")
            .then(u.clean);
      } catch (e) {
        u.alertError(e, msg);
      }
  ...
  .addCommand({
    ...
    usage: "[@user] | bhid",
    info:
      "Retrieves stats for a mentioned user (if they have `claim`ed their profile) or a given Brawlhalla ID.",
    category: "Stats",
    process: async (msg, suffix) => {
      try {
        let userMentions = u.userMentions(msg);
          ...
          if (bhid) {
            try {
              ...
              if (stats.brawlhalla_id) {
                ...
                msg
                  .reply("I couldn't find any stats for that Brawlhalla ID.")
                  .then(u.clean);
            } catch (e) {
              u.alertError(e, msg);
            }
            ...
            msg
              .reply("you need to give me a valid Brawlhalla ID.")
              .then(u.clean);
        }
