...
function checkBoard(msg, region, board = "1v1") {
  ...
  if (region && alias[region.toLowerCase()]) region = alias[region.toLowerCase()];
  ...
  if (region && bh.regions[region.toLowerCase()]) {
    let embed = u.embed()
      ...
      .sort((a, b) => a.rank - b.rank)
		  .forEach(record => {
				embed.addField(u.decode(record.name), `Rank: ${record.rank}\nRating: ${record.rating}\nCurrently ${(((record.rating - record.previous) > 0) ? "climbing" : "falling")}.`, true);
			});
			if (embed.fields.length == 0)
				embed.setDescription(`No Top ${pages * 50} ${region.toUpperCase()} players have played in the ${board} queue within the last ${queueTime} minutes.`);
    ...
    msg.reply(`you need to tell me which region to check. (${Object.keys(bh.regions).map(r => r.toUpperCase()).join(", ")})`).then(u.clean);
  }
}
...
.addCommand({name: "queue",
  category: "Community",
  description: `Show which regional top ${pages * 5} players have been queuing ranked within the last ${queueTime} minutes.`,
  syntax: "<region>",
  aliases: ["queueing", "q", "que"],
  process: (msg, suffix) => checkBoard(msg, suffix, "1v1")
})
.addCommand({name: "queue2v2",
  category: "Community",
  description: `Show which regional top ${pages & 5} players have been queueing 2v2 ranked within the last ${queueTime} minutes.`,
  syntax: "<region>",
...
.setClockwork(() => {
  ...
  if (!shard || (shard.id === 0)) {
    ...
    return setInterval(() => {
      Module.db.leaderboards.fetchRankings(regions[i % regions.length], ((i % pages) + 1));
      i++;
      i %= iterations;
    }, (refresh / iterations));
  }
