...
function clanEmbed(clan) {
  let embed = u
    ...
    .setDescription(
      `__**Clan Information**__\n**Created:** ${new Date(
        clan.clan_create_date * 1000
  ...
  clan.clan
    ...
    .forEach((member) => {
      embed.addField(
        u.decode(member.name),
        `${member.rank} (${member.xp} xp)\nJoined ${new Date(
          member.join_date * 1000
        ).toLocaleDateString("en-US")}`,
        true
      );
    });

  if (clan.clan.length > 24)
    embed.addField(
      "More Members...",
      `All clan members are visible on [BrawlDB](https://brawldb.com/clan/info/${clan.clan_id})`
    );
  ...
  process: async (msg) => {
    try {
      ...
      if (
        ...
        } else if (stats && !stats.clan)
          msg.channel.send(`${stats.name} doesn't have a clan.`).then(u.clean);
        else
          msg.channel
            .send("I couldn't get any Brawlhalla info for " + target.username)
            .then(u.clean);
      } else if (user && !user.public)
        msg.channel
          .send(`${target.username}'s profile is not public.`)
          .then(u.clean);
      else
        msg.channel
          .send(`${target.username} hasn't claimed their profile yet!`)
          .then(u.clean);
    } catch (e) {
      u.alertError(e, msg);
    }
