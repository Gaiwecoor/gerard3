  ...
  process: async (msg) => {
    try {
      ...
      if (
        ...
        if (rank) {
          ...
          if (games >= 10) {
            ...
            let embed = u
              ...
              .setDescription(
                `${rank.name}'s estimated Glory and 1v1 rating after the ranked season reset are:`
              )
            ...
            u.botSpam(msg).send(
              msg.author +
                ", a player must play at least **10** ranked games each season to earn Glory!"
            );
        } else
          u.botSpam(msg).send(
            msg.author + ", I couldn't find ranked info for " + target.username
          );
        ...
        msg
          .reply(
            "you need to `claim` your profile first. Glory can also be calculated online, here: <http://www.brawlhalla.com/glory-calculator/>"
          )
          .then(u.clean);
      }
    } catch (e) {
