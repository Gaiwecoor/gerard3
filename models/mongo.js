...
const models = {
  claim: {
    getClanUsers: (clanMembers) => {
      return new Promise((fulfill, reject) => {
        Claim.find({ bhid: { $in: clanMembers } }, function (err, users) {
          if (err) reject(err);
          else fulfill(users);
        });
      });
    ...
    getTwitch: (channel) => {
      return new Promise((fulfill, reject) => {
        Claim.find({ twitch: channel }, function (error, users) {
    ...
    getUser: (discordId) => {
      return new Promise((fulfill, reject) => {
        Claim.findOne({ discordId: discordId }, function (err, user) {
          if (err) reject(err);
          else fulfill(user);
        });
      });
    ...
    saveUser: (data) => {
      return new Promise((fulfill, reject) => {
        ...
        if (!params.discordId)
          reject({ error: "'discordId' is a required field." });
        else {
    ...
    removeClaim: (discordId) => {
      return new Promise((fulfill, reject) => {
        Claim.findOneAndRemove({ discordId: discordId }, function (err, claim) {
          if (err) reject(err);
          else fulfill(claim);
        });
      });
          ...
          function (err, cmd) {
            if (err) reject(err);
              ...
              if (cmd.attachment) {
                ...
                request(data.url).pipe(
                  fs.createWriteStream(process.cwd() + "/storage/" + cmd._id)
                );
              }
              fulfill(cmd);
            }
        ...
        bh.getRankings({
          ...
          .then((rankings) => {
            rankings.forEach((ranking) => {
              let bhid = ranking.brawlhalla_id
                ? ranking.brawlhalla_id
                : ranking.brawlhalla_id_one + "+" + ranking.brawlhalla_id_two;
  ...
  server: {
    ...
    getSettings: (guildId) => {
      if (serverSettings.has(guildId)) return serverSettings.get(guildId);
      else {
        return new Promise((fulfill, reject) => {
